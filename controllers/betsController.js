...
async function getCurrentRound(req, res) {
  try {
    ...
    if (!round) {
      // Calculate current round based on IST time
      const now = new Date();
    ...
    // Current round winner (if already set)
...
/**
 * 2️⃣ Place Bet (Atomic, no race condition)
 */
...
/**
 * 3️⃣ My Bet History (for user, today's only)
 */
async function myBetHistory(req, res) {
  try {
    ...
    // Get winners for all rounds in this user's history
    ...
    // Group bets by round for response
    const roundMap = {};
    bets.forEach(bet => {
